...
export function createCoins(params, path, heightAt) {
  ...
  // ---- placement ----
  // March along the path, dropping each coin close to the centerline at a
  // comfortable flight height — right on the natural racing line, so sweeping
  // them up barely diverts the run. (No more wall/floor/bend detours: coins are
  // a light bonus on a timed race, not a risk-reward hunt.)
  const positions = [];
  ...
  for (let t = t0; t <= t1; t += CFG.coinSpacing * (0.75 + rnd() * 0.5)) {
    const lateral = (rnd() * 2 - 1) * hw * 0.15;   // barely off-centerline
    const height = 9 + rnd() * 6;                   // 9..15 m above the floor
